...
public class UserController extends BaseController {
    ...
    public String updateUser(@Valid @ModelAttribute("user") User user ){
        try{
            ...
            userEntity.setCreadedDate(user.getCreadedDate());
    ...
    public String changePassword(@Valid @ModelAttribute("productname") ProductVM productName,
        ...
        changePasswordVM.setLayoutHeaderVM(this.getLayoutHeaderVM(response, request, principal));
    ...
    public String changePasswordPost(@Valid @ModelAttribute("change-password") ChangePasswordVM pasword){

        try{
            if (pasword.getCurrentPassword()!=null && pasword.getNewPassword()!=null && pasword.getConfirmPassword()!=null){
                String username = SecurityContextHolder.getContext().getAuthentication().getName();

                User userEntity= userService.findUserByUsername(username);

                if(passwordEncoder.matches(pasword.getCurrentPassword(),userEntity.getPasswordHash())){
                    if (pasword.getNewPassword().equals(pasword.getConfirmPassword())){
                        userEntity.setPasswordHash(passwordEncoder.encode(pasword.getNewPassword()));
                        userService.updateUser(userEntity);
